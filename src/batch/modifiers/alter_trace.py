...
class AlterTraceAlignmentWraper(ABC):
    ...
    def alter_trace(self, execution, config):
        ...
        len_modification = self._remove_key(execution)

        position, result = self._alter_trace(len_modification, execution, config)
    ...
    def _remove_key(self, execution):
        result = execution.alg_current_config[f'{self.key}']
        execution.alg_current_config.pop(f'{self.key}')
        return result
